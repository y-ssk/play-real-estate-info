...
/**
 * 逐次ランプ・青（ColorBrewer Blues・5段 薄→濃）＝**浸水（洪水該当面積率）**の色相（`ADR-0032`）。
 *
 * 青は水害（浸水想定区域）の慣例色ゆえ、防災＝浸水分野の予約色として本ランプを新設する（`ADR-0032`
 * 配色体系）。従来 DESIGN §1 は「青は浸水の慣例色ゆえ他指標では避ける」としてきた＝その青を、いざ浸水指標
 * を出すときに正しく割り当てる。緑（相場）・紫（将来）・灰（基盤）・コーラル（UI 操作色）と色相で割れ、
 * トグル切替で「今は浸水指標」と識別できる。各段内は濃淡＝該当面積率の大小のみ（良し悪しを込めない・
 * `ADR-0005`）。**具体の見え方は層4目視で評価・確定**（暫定・`ADR-0032`結論6）。
 */
export const CHOROPLETH_FILL_RAMP_BLUE = [
  "#eff3ff",
  "#bdd7e7",
  "#6baed6",
  "#3182bd",
