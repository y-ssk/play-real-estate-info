...
/**
 * 絞り込みが1つでも効いているか（min/max いずれか指定された条件があるか）を判定する（④・ADR-0028）。
 *
 * なぜ純関数か：「条件ゼロ＝地図ハイライト非作動＝普通の色分け」の分岐を UI/フックから分離して固める
 * （非作動なら ChoroplethLayer に `matched` を張らせず色分けを保つ・frontend-conventions §8）。
 * 空条件（min/max とも undefined）は store が溜めない方針だが、型上は来うるため両端 undefined を不実効とみなす。
 *
 * @param conditions 指標キー→数値条件。
 */
export function hasActiveCondition(conditions: Record<string, MetricCondition>): boolean {
  return Object.values(conditions).some((c) => c.min !== undefined || c.max !== undefined);
}
