...
describe("metricValueRange（スライダー両端＝present 値の min/max・ADR-0028）", () => {
  it("present の数値だけから min/max を取る", () => {
    // A=11.6, B=10.2, C=20.4（present）。D/E はデータなし/秘匿＝除外。
    expect(metricValueRange([A, B, C, D, E])).toEqual({ min: 10.2, max: 20.4 });
  });

  it("符号付き（増減率）の負を含む値域も正しく取る", () => {
    // POP_B=-0.1, POP_A=0.05, POP_C=0.0。
    expect(metricValueRange([POP_A, POP_B, POP_C])).toEqual({ min: -0.1, max: 0.05 });
  });

  it("present 値が無ければ null（スライダーを出せない）", () => {
    expect(metricValueRange([D, E])).toBeNull();
    expect(metricValueRange([])).toBeNull();
  });

  it("value=0（present・該当なし）は端値として数える（データなしと混同しない・ADR-0011）", () => {
    const zero: MetricValue = { code: "13109", value: 0, status: "present" };
    expect(metricValueRange([zero, A])).toEqual({ min: 0, max: 11.6 });
  });
});
