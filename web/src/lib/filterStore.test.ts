...
describe("hasActiveCondition（条件ゼロ＝地図ハイライト非作動・ADR-0028）", () => {
  it("条件が無ければ false（普通の色分け地図）", () => {
    expect(hasActiveCondition({})).toBe(false);
  });

  it("min だけでも実効＝true", () => {
    expect(hasActiveCondition({ area_km2: { min: 10 } })).toBe(true);
  });

  it("max だけでも実効＝true", () => {
    expect(hasActiveCondition({ area_km2: { max: 20 } })).toBe(true);
  });

  it("両端 undefined の空条件しか無ければ false（溜まっても非作動扱い）", () => {
    expect(hasActiveCondition({ area_km2: { min: undefined, max: undefined } })).toBe(false);
  });
});
