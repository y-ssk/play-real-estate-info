...
| 2a | 値の道を実証（市区町村面積を面塗り） | F1/S1 | `metric_value`(新規)・`/api/choropleth/values?metric=area_km2`・`setFeatureState` | 面積で濃淡色分け／データなし色抜き。取得・鍵不要(既存geomから算出)。継ぎ目を層1可視で固める(層1値域/層4) | 🔲 **次** |
