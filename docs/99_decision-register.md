...
| 防災レイヤーのコード定義（凡例）の収集 | IF定義収集と並行可（素材タスク）。整数コード（微地形28区分/液状化6段階/指定主体区分/現象の種類/区域区分等）の意味は各APIの「原典データ項目」ページ依存で、XST001のインライン記載は例外。コード→名称の対応表が無いと層1検証・表示で詰む。発端: 防災コア11API構造化（`docs/api-if-spec/`） |
