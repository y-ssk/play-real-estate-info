...
- **識別＝Issue番号**（振り直さない・小数採番しない）。新スライスはIssueを立てて起票（テンプレ `.github/ISSUE_TEMPLATE/slice.md`）。
- **役割分担（重複させない＝メンテを軽く）**：
...
|---|---|---|---|---|---|
| 1 | 区の輪郭を地図に描く | F1/S1 | `/api/choropleth/geometry`(済) | 東京69区の輪郭が地図に出る（層4） | ✅ |
| 2a | 値の道を実証（市区町村面積を面塗り） | F1/S1 | `metric_value`(新規)・`/api/choropleth/values?metric=area_km2`・`setFeatureState` | 面積で濃淡色分け／データなし色抜き。取得・鍵不要(既存geomから算出)。継ぎ目を層1可視で固める(層1値域/層4) | ✅ |
...
| Skeleton（ローディング骨組み）共通部品化＝melta作法に揃える | **トリガー＝スケルトンを複数箇所で使う時／スライス3.6（動的挙動）でローディング整備時**。発端＝スライス3.5でカルテのタイトルにインラインのスケルトンバーを置いたが、melta作法から3点外れる根拠の薄い例外になっている：(1)色＝`--color-divider`(slate-100)だが `prohibited.md`:183「スケルトンは bg-slate-200 固定」(2)`skeleton-pulse`の動きが無い(静的バー)(3)ariaが`aria-label`で、`skeleton.md`の`aria-busy="true"`＋`role="status"`＋sr-onlyの作法でない。**是正＝Skeleton共通部品を作りmelta作法どおり**(slate-200系の用途トークン新設・pulse・aria-busy/role=status)に揃え、KartePanelのインライン版を置換。作法の例外は相応の理由がなければ残さない（オーナー方針） |
