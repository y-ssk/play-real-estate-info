...
| 4 | 条件で絞り込み→該当を地図/一覧 | F3・F7/S1 | `/values`(全指標を`useQueries`で取得・面塗りとキャッシュ共有)・`useChoroplethGeometry`(code/name)・`metrics.ts` registry(条件/列の母体)・filter store(Zustand `useFilterStore`・条件min/max＋並べ替え)・純関数`lib/filtering`(突き合わせ/AND絞り込み/安定並べ替え)・地図ハイライト(`setFeatureState({matched})`＋`CHOROPLETH_MATCHED_FILL_OPACITY_EXPR`非該当を淡く)・`FilterPanel`(③開閉式・左側＝カルテ右側と住み分け) | 条件AND(重みづけしない・`ADR-0012`)で該当を地図ハイライト＋並べ替え一覧表／**データなしは条件を満たさず除外・一覧は末尾**(`ADR-0011`)／0件はメッセージ／**MVPはクライアント側絞り込み**(69区ゆえ素直・server動的WHEREはv1/規模時=生SQL例外`ADR-0015/0017`)。層1=純関数test(突き合わせ・AND・データなし除外・並べ替え安定/末尾・空結果)・層4目視(条件→地図該当強調・列ヘッダで並べ替え) | 🔲 実装済(要DB起動の層4目視) |
...
| カルテ表示項目の精査（`ADR-0011` 骨組みの肉付け） | **指標が増えるスライスで**（段1④で起票・オーナー指摘）。現状カルテ/一覧は registry の見出し数値を平坦に並べるだけ。`ADR-0011` の骨組み＝**分野順序（災害→利便→相場→将来→教育）・各分野2段（見出し数値＋詳細）・相場の統計セット（中央値/平均/25-75%幅/件数）・推移グラフ**を、各分野の指標が揃うスライスで肉付けする。比較ビュー v1 はこの骨組みを再利用する前提ゆえ、肉付けの形（順序・2段・グラフ）は v1 を見据えて決める |
