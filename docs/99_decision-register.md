...
| 2b | 駅の合計乗降客数を面塗り（実データETL・XKT015） | F1・F2/S1 | XKT015タイルスイープ(z11・本土28枚)→LineString中点(代表点)→市区町村ST_Contains→属性安定キーでタイル跨ぎ重複排除→`S12_054=1`抽出→2023(`S12_057`)合計→`metric_value`。FEは指標トグル(面積↔乗降客数) | 駅乗降客数で色分け。取得はオーナー実行(鍵)。本丸ETL(層1手厚く＝上野=505392/押上=559147再現の純関数テスト・事後アサート) | 🔲 実装済(要オーナー実行＝fetch→ingest→確認) |
