...
| (c) 条件から | 指標・有無で条件指定 | 条件を満たすエリアを**地図にハイライト**→クリックでカルテ→ピン留め（独立した一覧表は持たない・`ADR-0028`） |
...
- **F3 条件で絞り込み**：市区町村の数値・件数・有無に条件（重みづけしない・`ADR-0012`）＝起点(c)。**「広く探す」段に徹し地図ハイライトで該当を示す→カルテ→ピンへ繋ぐ**（独立一覧表なし・`ADR-0028`）
- **F4 地名で逆引き**：都県・市区町村名→該当エリア＝起点(a)
- **F5 エリアカルテ**：選択した市区町村の詳細パネル（`ADR-0011`）
- **F6 ピン留め**：localStorage（`ADR-0012/0013`）
- **F7 並べ替え一覧表**：指標で街を並べ替え（`ADR-0012`）。**母体＝ピン留めした候補（「比較」段）＝ピン一覧に置く**（絞り込み該当全件の表ではない・`ADR-0028`）
...
| **S1** | 地図探索ビュー | ③開閉式（`DESIGN`§2）。F1〜F7 を**パネル/オーバーレイで内包（確定）**＝パネル内包で進める（別ページ遷移は採らない・§8 解決）。S1内はパネル開閉＝“状態”であって画面遷移ではない（`ADR-0018`） | MVP |
| └ | （S1内）絞り込みパネル | F3（地図ハイライト→カルテ→ピン・独立一覧表なし・`ADR-0028`）。**左の操作列のボタン（指標トグルの下）で開閉する左オーバーレイのドロワー＝PC固定・移動なし。開いてもボタンは残り（active）その直下にパネル・閉じはボタン再クリック/X/ESC（`ADR-0031`）／モバイル(スマホ幅)はボトムシート＝別スライス**（`ADR-0029`・`DESIGN`§2／カルテは右側固定） | MVP |
| └ | （S1内）レイヤー/凡例 | F1/F2 | MVP |
| └ | （S1内）カルテパネル | F5（**実装済**＝③開閉式・右側固定パネル・`DESIGN`§2／選択は Zustand・`ADR-0018`） | MVP |
| └ | （S1内）ピン一覧／並べ替え | F6/F7（**並べ替え一覧表(F7)はここ＝ピン候補の「比較」段**・`ADR-0028`） | MVP |
...
flowchart TD
  ...
  S1 -->|"市区町村を選択 / 条件(c)で該当をハイライト→クリック"| K["カルテパネル（S1内）"]
  K -->|"ピン留め"| L["ピン一覧・並べ替え（S1内）"]
  S1 -.->|"about・出典"| S2
  L -->|"v1"| S3["S3 比較ビュー"]
  K -->|"v1"| S3
```

- **S1 の内部はパネル開閉＝“状態”であって画面遷移ではない**（`DESIGN`§2の「出し入れ」・**パネル内包で確定**＝§8 解決）。選択中の単位は Zustand（`ADR-0018`）、カルテパネルはその選択に従って開閉する（地図クリックで開く・クローズで地図全面へ）。
- **絞り込み(c)は dead-end な一覧で終わらせない**（`ADR-0028`）：条件で該当区を地図ハイライト→クリックでカルテ→ピン留め→ピン一覧で並べ替え比較。「広く探す→ピン→比較」（`docs/01`§3）に乗せる。並べ替え一覧表(F7)は絞り込み側でなくピン一覧側に置く。
...
| S1 面塗り（値） | `GET /api/choropleth/values?metric=`＋`setFeatureState`（`ADR-0016`） | ✅ 実装済（段1スライス2a/2c） |
...
| F5 カルテ | `GET /api/karte?unit_id=`（1単位×全指標・`ADR-0011`） | ✅ 実装済（段1スライス③） |
...
| ~~S1 の内部構成（パネル内包 vs 別ページ遷移）~~ | **解決＝パネル内包**（段1スライス③実装時） | カルテパネルを③開閉式（`DESIGN`§2）で実装＝パネル開閉は“状態”（`ADR-0018`・選択 Zustand）であって画面遷移ではない。別ページ遷移は採らない＝§4 遷移図はそのまま有効 |
| ~~カルテの置き方~~ | **解決＝S1内・右側固定パネル**（PC） | カルテパネル（F5）採用＝`DESIGN`§2 開いた状態。モバイルはボトムシート（PC実装後・申し送り済） |
