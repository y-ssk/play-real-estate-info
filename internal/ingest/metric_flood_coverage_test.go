...
// TestParseFloodTile_KeepsPolygon は、Polygon の geometry を只取りして GeoJSON 文字列で返すことを確かめる
// （coordinates を Go でパースし直さず、ST_GeomFromGeoJSON が解釈できる形で持つ・§4）。
func TestParseFloodTile_KeepsPolygon(t *testing.T) {
	t.Parallel()
	polys, err := parseFloodTile(strings.NewReader(floodTile(
		floodFeat("Polygon", simplePolyCoords),
	)))
	if err != nil {
		t.Fatalf("parseFloodTile: %v", err)
	}
	if len(polys) != 1 {
		t.Fatalf("採用ポリゴン=%d want 1", len(polys))
	}
	gj := polys[0].geoJSON
...
// TestParseFloodTile_RejectsNonPolygon は、Polygon 以外の geometry（想定外の型）を採らないことを確かめる。
// XKT026 は Polygon（IF §3）ゆえ MultiPolygon 等が来たら取り違えとして落とす（防御）。
func TestParseFloodTile_RejectsNonPolygon(t *testing.T) {
	t.Parallel()
	polys, err := parseFloodTile(strings.NewReader(floodTile(
	...
	if len(polys) != 1 {
		t.Fatalf("採用ポリゴン=%d want 1（Polygon のみ）", len(polys))
	}
}

// TestParseFloodTile_RejectsMissingCoords は、coordinates 欠落（面を成さない）を採らないことを確かめる。
func TestParseFloodTile_RejectsMissingCoords(t *testing.T) {
	t.Parallel()
	// coordinates キーを持たない geometry（type だけ）。空 coordinates を弾く。
	feat := `{ "type":"Feature", "geometry": { "type":"Polygon" }, "properties": {} }`
	polys, err := parseFloodTile(strings.NewReader(floodTile(feat)))
	if err != nil {
		t.Fatalf("parseFloodTile: %v", err)
	}
	if len(polys) != 0 {
		t.Fatalf("採用ポリゴン=%d want 0（座標欠落は棄却）", len(polys))
	}
}

// TestParseFloodTile_MultiFeatures は、1タイル内の複数 Polygon を全て返すことを確かめる（重複排除しない）。
// タイル分割・境界重複の断片も全て載せ、結合は SQL の ST_Union に委ねる設計（§4・R1）。
func TestParseFloodTile_MultiFeatures(t *testing.T) {
	t.Parallel()
	polys, err := parseFloodTile(strings.NewReader(floodTile(
		floodFeat("Polygon", simplePolyCoords),
		floodFeat("Polygon", simplePolyCoords), // 同形（重複でも排除しない）
		floodFeat("Polygon", simplePolyCoords),
	)))
	if err != nil {
		t.Fatalf("parseFloodTile: %v", err)
	...
	if len(polys) != 3 {
		t.Fatalf("採用ポリゴン=%d want 3（結合は SQL 側ゆえパース段では排除しない）", len(polys))
	}
}

// TestParseFloodTile_RejectsNonFeatureCollection は、エラー応答（鍵切れ等）を取り違えないことを確かめる。
func TestParseFloodTile_RejectsNonFeatureCollection(t *testing.T) {
	t.Parallel()
	if _, err := parseFloodTile(strings.NewReader(`{"statusCode":401,"message":"Access denied"}`)); err == nil {
